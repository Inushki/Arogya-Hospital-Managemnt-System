...
if(isset($POST['txtDocID']))
{
	$patid=$_POST['txtDocID'];
}

$conn=mysqli_connect('localhost','root','','hnd_027_proj');
if($conn){
if(!empty($docid)){
$queData="SELECT `DocID` , `DName` , `DName` , `NIC` , `ConNo` , `Email` , `Gender` , `CivilStat` , `DAdress`
FROM `doctor` WHERE DocID = '$docid'";
...
<table border="0" width="95%" cellspacing="0" cellpadding="8" align="center">
	<tr>
		<th bgcolor="#000066">Doctor ID</th>
...
echo '
	<tr>
		...
		<td>'.$Vals['DAdress'].'</td>
		<td><a href="DocEdit.php?id='.$docid.'"><img src="pencil.png" width="30" 
		height="30"></a></td>
		<td><a href="DocDelete.php?id='.$docid.'"><img src="trash.png" width="30" 
		height="30"></a></td>
	</tr>';
}
'</table>
...
echo 'Doctor ID:'.$docid.' NOT AVAILABLE in the Database!<br> Check 
the Doctor ID & Retry!';
}
}
else{
echo 'Error while Search for Doctor Details!',mysqli_error($conn);
}
}
else{
$queData="SELECT * FROM doctor";
if(mysqli_query($conn,$queData)){
$resRec=mysqli_query($conn,$queData);
if(mysqli_num_rows($resRec)>0){
...
<td>'.$Vals['CivilStat'].'</td>
<td>'.$Vals['DAdress'].'</td>
<td><a href="DocEdit.php?id='.$Vals['DocID'].'"><img src="pencil.png" width="30" 
height="30"></a></td>
<td><a href="DocDelete.php?id='.$Vals['DocID'].'"><img src="trash.png" width="30" 
height="30"></a></td>
</tr>';
}
echo '</table>
...
echo 'Doctor ID:'.$docid.' NOT AVAILABLE in the Database!<br> 
Check the Doctor ID & Retry!<br><a href="PatientSearch.php"><img src="back.png" width="50" 
height="50"></a>';
}
}
else{
