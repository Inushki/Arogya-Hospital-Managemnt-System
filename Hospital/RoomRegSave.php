...
<?php
     ...
     $rno=$_POST['txtRNo'];
	 $rtype=$_POST['txtRType'];
	 $st=$_POST['txSt'];
	 
    //Use the below line to verify the data passing between php pages
    //echo 
 $rname.'<br>'.$uname.'<br>'.$pw;

      $conn=mysqli_connect('localhost','root','','hnd_027_proj');
      if($conn){
              $queSave="INSERT INTO 
 user(RoomNo,RoomType,Status) 
 VALUES('$rno','$rtyoe','$st')";
 
            if(mysqli_query($conn,$queSave)){
               echo '<tr>
                        <td align="center">
                        You have Successfully Save '.$rno.' to the Database!
                        </td>
                     </tr>
                     <tr>
                        <td align="center"><a href="userreg.php"><img src="Image/back.png" width="50" height="50"></a></td>
                     </tr>
                     <tr>
