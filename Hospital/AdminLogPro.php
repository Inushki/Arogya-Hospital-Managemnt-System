...
<?php
	...
	if(!empty($un) && !empty($pw)){
       $con=mysqli_connect('localhost','root','','hnd_27_first');
       
       if($con){
          $queLog="SELECT * FROM user_table WHERE UserName='$un'";
          if(mysqli_query($con,$queLog)){
               $DataRec=mysqli_query($con,$queLog);
                   ...
                   if($RowVal["UserName"]==$un && $RowVal['Password']==$pw){
                      ...
                      $queInSch="INSERT INTO login_shedule(UserName,LogIn) VALUES('$un',(now()))";
